# Model Prompt Engineering Guide
...
Here is a good starting point for structuring your prompts hierarchy.
...
**Delimiters** are markers that organize different parts of a prompt or response.
...
![Context window, cost and speed comparison](/assets/images/context-window-comparison.png)
...
"Let me define our key components:
- UserService: handles authentication and profile management
- OrderService: processes purchases and payments
- InventoryService: manages product stock

Now, when we discuss UserService optimization, we'll have clear context."
...
```
"To summarize our discussion so far: we've established a database schema for user profiles, implemented basic CRUD operations, and now we're working on adding search functionality. Next, I'd like to discuss indexing strategies."
```

**Selective Context Retention**

Not all data from your thread remains important to your conversation. Reference or repeat only the critical pieces of context to steer the model and avoid unintended associations. This practice is particularly effective when switching topics, refining previous outputs, or focusing the model’s attention on a specific objective.
An example of how to perform this is below.

```
"From our earlier conversation, the key technical decisions were: React frontend, Express backend, MongoDB database. Skipping the deployment discussion for now - let's focus on implementing the user profile update feature."
```
...
```
"Before we implement this solution, let me confirm: you understand I need real-time updates pushed to the client, not polling-based updates, correct?"
```
...
ChatGPT models are not particularly good at retaining uploaded files in memory; when working with context from such files, it is best to copy and paste any relevant text into the chat. When building prompt agent systems, you can upload the PDF "[**The Prompt Report: A Systematic Survey of Prompting Techniques**](https://learning.coach/wp-content/uploads/2024/06/2024-The-Prompt-Report-A-Systematic-Survey-of-Prompting-Techniques.pdf)" to your system prompt to provide the model with better context.
