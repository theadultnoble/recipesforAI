...
A solid starting point for structuring your prompt hierarchy comes from OpenAI’s GPT-4.1 prompt guide. This structure works reliably across any generative model.
...
It is fair to note that for simpler well known task you could exclude examples within your prompt. LLMs are trained on a vast range of patterns and tasks. With good phrasing, they can generalize well even without seeing specific examples.

However, for more niche task, providing examples will output better results. The goal is to guide the model’s behavior by showing what kind of response you expect. You're leveraging in-context learning—the model learns the pattern from examples and tries to continue it. This method of prompting is called Few-Shot prompting.
